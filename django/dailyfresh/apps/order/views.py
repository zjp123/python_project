...
# order/place
class OrderPlace(LoginCheck, View):
    def post(self, req):
        user = req.user
        cart_key = 'cart_%d'%user.id
        # 获取数据
        skuids = req.POST.getlist('sku_ids')
        if not skuids:
            return JsonResponse({'res': 0, 'message': 'data not wanzheng'})
        ...
        for id in skuids:
           ...
           sku.count = count
           xiaoji = int(count)*sku.price
           sku.xiaoji = xiaoji

           total_jianshu += int(count)
           total_price += xiaoji
           skus.append(sku)
        ...
        # 实付款
        total_pay = trans_price + total_price
        str_ids = ','.join(skus)
        context = {
            'skuids':skus,
...
# order/commit
# param addr_id pay_method sku_ids
...
class CommentView(LoginCheck, View):
    """订单评论"""
